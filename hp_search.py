...
def hyperparameter_searching(conf):
    ...
    for i in tqdm(building_num):
        X_train = new_train_df[new_train_df["building_num"] == i].drop(
            ["building_num", "power"], axis=1
        )
        y_train = new_train_df[new_train_df["building_num"] == i]["power"]

        X_valid = new_valid_df[new_valid_df["building_num"] == i].drop(
            ["building_num", "power"], axis=1
        )
        ...
        gcv = GridSearchCV(
            estimator=XGBRegressor(
                tree_method="hist", objective=loss_fn.weighted_mse(1)
            ),
            param_grid=grid,
            cv=5,
            refit=True,
