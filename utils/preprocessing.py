...
def train_pre_processing(train_df: pd.DataFrame) -> pd.DataFrame:
    ...
    train_df = pd.merge(
        train_df, day_hour_power_mean, how="left", on=["building_num", "hour", "day"]
    )

    #  day_hour_std 변수 추가
    day_hour_power_std = preprocessing_fn.create_day_hour_std(train_df)
    train_df = pd.merge(
        train_df, day_hour_power_std, how="left", on=["building_num", "hour", "day"]
    )

    # 컬럼 제거
    # num_date_time, date, sunshine, solar, hour drop 컬럼 제거
    train_df = train_df.drop(
        ["num_date_time", "date", "sunshine", "solar", "hour"], axis=1
    )
...
def test_pre_processing(test_df, train_df=None):
    ...
    day_hour_power_mean = preprocessing_fn.create_day_hour_mean(train_df)
    test_df = pd.merge(
        test_df, day_hour_power_mean, how="left", on=["building_num", "hour", "day"]
    )

    # day_hour_std 변수 추가
    day_hour_power_std = preprocessing_fn.create_day_hour_std(train_df)
    test_df = pd.merge(
        test_df, day_hour_power_std, how="left", on=["building_num", "hour", "day"]
    )
